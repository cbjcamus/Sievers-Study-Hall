...
SYNONYMS_PATH = os.path.join(BASE_DIR, "utils", "synonyms.csv")
...
def get_synonyms(answer, exercise, file_path=SYNONYMS_PATH):
    ...
    # Filter rows where exercise matches and output matches the word
    filtered = df[(df['exercise'] == exercise) & (df['output'] == answer)]

    # Get the input column and join it as a comma-separated string

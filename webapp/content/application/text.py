# EXERCISE PAGE

EXERCISE_TITLE = {
    'english': 'Exercise',
    'french': 'Exercice',
}

ENTER_ANSWER_HERE = {
    'english': 'Enter answer here',
    'french': 'Écrivez la réponse ici',
}

YOUR_ANSWER = {
    'english': 'Your answer:',
    'french': 'Votre réponse :',
}

ALL_QUESTIONS_SUCCESSFULLY_ANSWERED = {
    'english': 'All questions have been successfully answered!',
    'french': 'Toutes les questions ont été répondue avec succès !',
}

YOUR_SCORE_FOR_THIS_EXERCISE = {
    'english': 'Your score for this exercise is',
    'french': 'Votre score pour cet exercice est',
}

FEEDBACK_LAST_QUESTION = {
    'english': 'Feedback last question:',
    'french': 'Feedback dernière question :',
}

YOUR_INCORRECT_ANSWERS = {
    'english': 'Your incorrect answers:',
    'french': 'Vos réponses incorrectes :',
}

ADDITIONAL_HELP = {
    'english': 'Additional Support',
    'french': 'Informations Complémentaires',
}

CONSULT_FAQ = {
    'english': 'For additional support consult the <a href="https://sieversstudyhall.substack.com/p/faq" target="_blank">FAQ</a> or send me an email at sieversstudyhall@gmail.com',
    'french': 'Pour plus d\'information, consultez le <a href="https://sieversstudyhall.substack.com/p/faq-8cc" target="_blank">FAQ</a> ou envoyez moi un email à sieversstudyhall@gmail.com'
}

NOT_AUTHENTICATED = {
    'english': 'You are not logged-in. Sign-up to save your progress:',
    'french': 'Vous n\'êtes pas connecté. Créez un compte pour enregistrer votre progrès:',
}

# HOME PAGE

META_DESCRIPTION = {
    'english': 'Practice German with interactive exercises. Improve your grammar and get instantaneous feedback, for free.'
               'Practice Präpositionen, Artikel, Pronomen, Konnektoren, Adverbien, Genus, Partizip II, Präteritum and much more.',
    'french': 'Pratiquez l’allemand avec des exercices interactifs. Améliorez votre grammaire et recevez un feedback immédiat, gratuitement.'
              'Practiquez les Präpositionen, Artikel, Pronomen, Konnektoren, Adverbien, Genus, Partizip II, Präteritum et bien plus.',
}
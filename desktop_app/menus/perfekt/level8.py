...
DATA_PATH = DATA_PATH[perfekt]
SCORE_PATH = f"{SCORE_PATH[perfekt]}/level8.csv"


class PerfektLevel8(tk.Frame):
    def __init__(self, parent, controller):
        ...
        create_button_back(self, "Back to Perfekt",
                           lambda: controller.show_frame(Perfekt))
    ...
    def get_new_question(self):
        if not self.data.empty:
            ...
            question = (f"Find the Partizip II of the following verb:"
    ...
    def check_answer(self):
        ...
        correct_answer = self.current_question['perfekt'].strip()

        if is_equal(user_answer, correct_answer, self.exercise):
            self.result.config(text="Correct", fg="green")
            text = f"Partizip II {self.current_question['german']}: {self.current_question['perfekt']}"

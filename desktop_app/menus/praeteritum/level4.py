...
SCORE_PATH = f"{SCORE_PATH[praeteritum]}/level4.csv"


class PraeteritumLevel4(tk.Frame):
    def __init__(self, parent, controller):
        ...
        create_button_back(self, "Back to Präteritum",
    ...
    def get_new_question(self):
        if not self.data.empty:
            ...
            question = (f"Find the präteritum (3rd Person Singular) of the following verb:"
    ...
    def check_answer(self):
        ...
        correct_answer = self.current_question['er/sie/es'].strip()

        if is_equal(user_answer, correct_answer, self.exercise):
            self.result.config(text="Correct", fg="green")
            text = f"{self.current_question['german']} → er/sie/es {self.current_question['er/sie/es']}"

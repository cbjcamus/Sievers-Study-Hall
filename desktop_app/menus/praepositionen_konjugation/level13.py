...
from desktop_app.interface.widgets import (create_title, create_button_back, create_subtitle, create_entry, create_submit_button,
                                           create_result_text, create_feedback_text, create_proverb_text, create_refresh_button,
...
DATA_PATH = DATA_PATH[praepositionen_konjugation]
SCORE_PATH = f"{SCORE_PATH[praepositionen_konjugation]}/level13.csv"


class PraepositionenKonjugationLevel13(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.level = 13
        self.exercise = praepositionen_konjugation
        ...
        create_button_back(self, "Back to Präpositionen",
                           lambda: controller.show_frame(PraepositionenKonjugation))
    ...
    def get_new_question(self):
        if not self.data.empty:
            ...
            question = (f"Find the präposition that match the following noun:"
                        f"\n\n{self.current_question['english_verb']} = {self.current_question['verb']} _____")
    ...
    def check_answer(self):
        ...
        if is_equal(user_answer, correct_answer, self.exercise):
            ...
            text = (f"{self.current_question['english_verb']}"
                    f" = {self.current_question['verb']} {self.current_question['preposition']}")

...
from desktop_app.interface.style import bg_color, bullet
...
DATA_PATH = DATA_PATH[adjektivDeklinationen]
SCORE_PATH = f"{SCORE_PATH[adjektivDeklinationen]}/level4.csv"

class AdjektivdeklinationenLevel4(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.level = 4
        self.exercise = adjektivDeklinationen
        ...
        create_button_back(self, "Back to Adjektivdeklinationen",
                           lambda: controller.show_frame(Adjektivdeklinationen))
    ...
    def get_new_question(self):
        if not self.data.empty:
            ...
            question = (f"Complete the sentence with the correct form of the adjektiv specified:"
                        f"\n\n{self.current_question['question']} {bullet} {self.current_question['adjectiv']}")
    ...
    def check_answer(self):
        ...
        if is_equal(user_answer, correct_answer, self.exercise):
            ...
            text = (f"{self.current_question['sentence']}"
                    f"\n\n{self.current_question['article']}, "
                    f"{self.current_question['gender']}, "
                    f"{self.current_question['case']} → "
                    f"{self.current_question['answer']}")
    ...
    def update_score(self):
        self.scores.loc[self.data["Nr"] == self.current_question["Nr"], "score"] = 1

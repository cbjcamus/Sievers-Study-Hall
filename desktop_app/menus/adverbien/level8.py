...
from desktop_app.interface.widgets import (create_title, create_button_back, create_subtitle, create_entry, create_submit_button, create_result_text, create_feedback_text,
...
DATA_PATH = DATA_PATH[adverbien]
SCORE_PATH = f"{SCORE_PATH[adverbien]}/level8.csv"

class AdverbienLevel8(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.level = 8
        self.exercise = adverbien

        create_title(self, "Adverbien")
        create_subtitle(self, f"Level {self.level}")

        self.question = create_question_text(self)
        self.question_lines = 3

        self.entry = create_entry(self, question_lines=self.question_lines)

        self.submit_button = create_submit_button(self, command=self.check_answer, question_lines=self.question_lines)

        self.result = create_result_text(self, question_lines=self.question_lines)

        self.feedback = create_feedback_text(self, question_lines=self.question_lines)

        self.proverb = create_proverb_text(self)

        create_button_back(self, "Back to Adverbien",
                           lambda: controller.show_frame(Adverbien))
    ...
    def get_new_question(self):
        if not self.data.empty:
            ...
            question = (f"Find the missing adverb in the sentence:"
    ...
    def check_answer(self):
        ...
        if is_equal(user_answer, correct_answer, self.exercise):
            ...
            text = (f"{self.current_question['german']}"
                    f"\n\n{self.current_question['english']}")
    ...
    def show_refresh_button(self):
        self.refresh_button = create_refresh_button(self, command=self.reset_scores, question_lines=self.question_lines)

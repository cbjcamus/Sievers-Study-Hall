...
DATA_PATH = DATA_PATH[verben]
SCORE_PATH = f"{SCORE_PATH[verben]}/level8.csv"


class VerbenLevel8(tk.Frame):
    def __init__(self, parent, controller):
        ...
        create_button_back(self, "Back to Verben",
                           lambda: controller.show_frame(Verben))
    ...
    def get_new_question(self):
        if not self.data.empty:
            ...
            question = (f"Find the german verb that fits:"
                        f"\n\n{self.current_question['adjective_english']} → {self.current_question['english']}"
                        f"\n\n{self.current_question['adjective_german']} → _____")
    ...
    def check_answer(self):
        ...
        correct_answer = self.current_question['german'].strip()

        if is_equal(user_answer, correct_answer, self.exercise):
            self.result.config(text="Correct", fg="green")
            text = (f"\n\n{self.current_question['adjective_english']} → {self.current_question['english']}"
                    f"\n\n{self.current_question['adjective_german']} → {self.current_question['german']}")
